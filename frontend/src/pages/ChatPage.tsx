...
export function ChatPage() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      for await (const chunk of streamChatMessageFetch({
        ...
        // First chunk might contain session ID
  ...
  return (
    <div className="flex gap-4 h-[calc(100vh-7rem)]">
      {/* Sidebar: sessions */}
      <div className="w-48 shrink-0 bg-gray-800 rounded-xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto">
          {sessions.map((s) => (
            <div
              key={s.id}
              className={`group flex items-center justify-between px-3 py-2 cursor-pointer hover:bg-gray-700 transition-colors ${
                s.id === activeSessionId ? "bg-gray-700" : ""
              }`}
      ...
      <div className="flex-1 flex flex-col bg-gray-800 rounded-xl overflow-hidden">
            ...
            {(["general", "course_only"] as const).map((mode) => (
              <button
                ...
                className={`px-2 py-1 rounded text-xs font-medium transition-colors ${
                  knowledgeMode === mode ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-400 hover:bg-gray-600"
                }`}
              >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {streaming && !streamingText && (
            <div className="flex justify-start">
              <div className="bg-gray-700 rounded-xl px-4 py-2.5 text-sm text-gray-400">
                {t("chat.thinking")}
              </div>
            </div>
          )}
